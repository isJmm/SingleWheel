...
IFX_INTERRUPT(cc60_pit_ch1_isr, 0, CCU6_0_CH1_ISR_PRIORITY)
{
    ...
    if(0 == (time % 1)) {
        if (angleUsed < angle_offset) {
            angleUsed += 0.5f;
        }
        else if (angleUsed > angle_offset) {
            angleUsed -= 0.5f;
        }

        if (fangxiang_flag == 1) Dynamic_zero_set = Dynamic_zero_cale();
        else if (fangxiang_flag == 2) Dynamic_zero_set = -Dynamic_zero_cale();
        else Dynamic_zero_set = 0;
    }
